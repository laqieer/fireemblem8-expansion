    ...
    def test_patch_release_raw_cgroup_membership_reads_reject_after_identity_check(
        ...
        with (
            ...
            for (
                label,
                changed,
            ) in patch_workflow_tests.generate_membership_checker_control_flow_mutations(
                original
            ):
                with self.subTest(
                    membership_checker_control_flow=label
                ), self.assertRaisesRegex(
                    ValueError,
            ...
            for (
                label,
                changed,
            ) in patch_workflow_tests.generate_mandatory_operator_mutations(
                original
            ):
                with self.subTest(
            ...
            for (
                label,
                changed,
            ) in patch_workflow_tests.generate_generic_heredoc_spoof_mutations(
                original
            ):
                with self.subTest(
                    generic_heredoc_spoof=label
                ), self.assertRaisesRegex(
                    ValueError,
                    "isolated candidate build differs",
                ):
            ...
            for (
                label,
                changed,
            ) in patch_workflow_tests.generate_commented_heredoc_hide_mutations(
                original
            ):
                with self.subTest(
            ...
            for (
                label,
                changed,
            ) in patch_workflow_tests.generate_arithmetic_command_comment_hide_mutations(
                original
            ):
                with self.subTest(
                    arithmetic_command_comment=label
                ), self.assertRaisesRegex(
                    ValueError,
                    "isolated candidate build differs",
                ):
            ...
            for (
                label,
                changed,
            ) in patch_workflow_tests.generate_ansi_arithmetic_controls(
                original
            ):
