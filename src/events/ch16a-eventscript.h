#include "gbafe.h"

CONST_DATA EventListScr EventScr_Ch16a_BeginningScene[] = {
    CALL(EventScr_Ch16A_11)
    SVAL(EVT_SLOT_B, 0x0)
    LOMA(CHAPTER_E_16)
    LOAD1(0x0, UnitDef_Ch15AAlly_2)
    ENUN
    LOAD1(0x1, UnitDef_Ch16AEnemy_0)
    ENUN
    SVAL(EVT_SLOT_2, UnitDef_Ch16AEnemy_1)
    SVAL(EVT_SLOT_3, 0x1)
    CALL(EventScr_LoadUnitForTutorial)
    CLEA
    CALL(EventScr_CommonPrep)
    ENUT(12)
    ENDA
};

CONST_DATA EventListScr EventScr_Ch16A_11[] = {
    MUSC(SONG_47)
    SVAL(EVT_SLOT_B, 0x0)
    LOMA(CHAPTER_40)
    FADU(16)
    LOAD2(0x1, UnitDef_Ch16AAlly_5)
    ENUN
    CUMO_CHAR(CHARACTER_EPHRAIM)
    STAL(60)
    CURE
    SetBackground(BG_PLAIN_2)
    TEXTSHOW(0xb39)
    TEXTEND
    MUSC(SONG_TENSION)
    TEXTCONT
    TEXTEND
    REMA
    FADI(16)
    CLEAN
    FADU(16)
    MOVE_1STEP(0x10, CHARACTER_MYRRH, FACING_UP)
    ENUN
    SetBackground(BG_PLAIN_2)
    TEXTSHOW(0xb3a)
    TEXTEND
    REMA
    MUSCMID(SONG_SILENT)
    FADI(16)
    CLEA
    CLEE
    CLEN
    SVAL(EVT_SLOT_B, 0xc0000)
    LOMA(CHAPTER_42)
    UNIT_COLORS(0x2)
    LOAD2(0x1, UnitDef_Ch16AAlly_6)
    ENUN
    FADU(16)
    LOAD2(0x1, UnitDef_Ch16AAlly_7)
    ENUN
    SOUN(SONG_B1)
    TILECHANGE(0x0)
    MOVE(0x0, CHARACTER_ORSON, 7, 6)
    ENUN
    TILEREVERT(0x0)
    LOAD2(0x1, UnitDef_Ch16AMixed_0)
    ENUN
    FADI(16)
    CLEA
    CLEE
    CLEN
    CHECK_MODE
    SVAL(EVT_SLOT_1, 0x2)
    BNE(0x1, EVT_SLOT_C, EVT_SLOT_1)
    SetBackground(BG_THRONE_NORMAL)
    TEXTSHOW(0xb3b)
    TEXTEND
    REMA
    FADI(16)
    CLEAN
LABEL(0x1)
    CHECK_MODE
    SVAL(EVT_SLOT_1, 0x2)
    BEQ(0x2, EVT_SLOT_C, EVT_SLOT_1)
    MUSC(SONG_SOLVE_THE_RIDDLE)
    EvtSetLoadUnitNoREDA // ENOSUPP in EAstdlib
    LOAD2(0x1, UnitDef_Ch16AAlly_8)
    ENUN
    REMU(CHARACTER_LYON)
    REMU(CHARACTER_RIEV)
    FADU(16)
    SVAL(EVT_SLOT_2, 0x40)
    MOVE_CLOSEST(0xffff, CHAR_EVT_SLOT2, 7, 11)
    CALL(EventScr_UnitWarpIN)
    SVAL(EVT_SLOT_2, 0x57)
    MOVE_CLOSEST(0xffff, CHAR_EVT_SLOT2, 8, 11)
    CALL(EventScr_UnitWarpIN)
    LOAD2(0x1, UnitDef_Ch16AAlly_8)
    ENUN
    CUMO_CHAR(0x80)
    STAL(60)
    CURE
    Text_BG(BG_CASTLE_BRIGHT, 0xb3c)
    MOVE_1STEP(0x10, 0x80, FACING_LEFT)
    MOVE_1STEP(0x10, 0x81, FACING_RIGHT)
    ENUN
LABEL(0x2)
    CHECK_MODE
    SVAL(EVT_SLOT_1, 0x2)
    BNE(0x3, EVT_SLOT_C, EVT_SLOT_1)
    LOAD2(0x1, UnitDef_Ch16AAlly_8)
    ENUN
    MOVE_1STEP(0xffff, 0x80, FACING_LEFT)
    MOVE_1STEP(0xffff, 0x81, FACING_RIGHT)
    FADU(16)
LABEL(0x3)
    LOAD2(0x1, UnitDef_Ch16AAlly_9)
    ENUN
    SOUN(SONG_B1)
    TILECHANGE(0x0)
    MOVE(0x10, CHARACTER_LYON, 7, 5)
    ENUN
    MOVE(0x10, CHARACTER_RIEV, 8, 6)
    ENUN
    TILEREVERT(0x0)
    LOAD2(0x1, UnitDef_Ch16AAlly_10)
    ENUN
    MOVE_1STEP(0x10, 0x80, FACING_RIGHT)
    ENUN
    MOVE_1STEP(0x10, 0x81, FACING_LEFT)
    ENUN
    MUSC(SONG_SOLVE_THE_RIDDLE)
    CUMO_CHAR(0x80)
    STAL(60)
    CURE
    SetBackground(BG_CASTLE_BRIGHT)
    CHECK_MODE
    SVAL(EVT_SLOT_1, 0x2)
    BNE(0x4, EVT_SLOT_C, EVT_SLOT_1)
    TEXTSHOW(0xb3d)
    TEXTEND
    GOTO(0x5)
LABEL(0x4)
    TEXTSHOW(0xb3e)
    TEXTEND
    MUSCMID(SONG_SILENT)
LABEL(0x5)
    REMA
    FADI(16)
    CLEA
    CLEE
    CLEN
    SetBackground(BG_THRONE_NORMAL)
    CHECK_MODE
    SVAL(EVT_SLOT_1, 0x2)
    BNE(0x6, EVT_SLOT_C, EVT_SLOT_1)
    TEXTSHOW(0xb3f)
    TEXTEND
    GOTO(0x7)
LABEL(0x6)
    MUSC(SONG_LOST_HEART)
    TEXTSHOW(0xb40)
    TEXTEND
LABEL(0x7)
    REMA
    FADI(16)
    CHECK_MODE
    SVAL(EVT_SLOT_1, 0x2)
    BNE(0x8, EVT_SLOT_C, EVT_SLOT_1)
    CLEAN
    CAMERA(0, 0)
    MUSC(SONG_LOST_HEART)
    FADU(16)
    LOAD2(0x1, UnitDef_Ch16AAlly_11)
    ENUN
    CUMO_CHAR(CHARACTER_RIEV)
    STAL(60)
    CURE
    Text_BG(BG_THRONE_NORMAL, 0xb41)
    SVAL(EVT_SLOT_2, 0x40)
    CALL(EventScr_UnitWarpOUT)
    SVAL(EVT_SLOT_2, 0x57)
    CALL(EventScr_UnitWarpOUT)
    FADI(16)
    CLEA
    CLEE
    CLEN
LABEL(0x8)
    UNIT_COLORS(0x0)
    ENDA
};

CONST_DATA EventListScr EventScr_Ch16A_0[] = {
    CALL(EventScr_Ch16A_12)
    MNCH(0x11)
    ENDA
};

CONST_DATA EventListScr EventScr_Ch16A_12[] = {
    FADI(16)
    CLEA
    CLEE
    CLEN
    MUSC(SONG_INTO_THE_SHADOW_OF_VICTORY)
    SetBackground(BG_CASTLE_INTERIOR)
    TEXTSHOW(0xb49)
    TEXTEND
    REMA
    FADI(16)
    SVAL(EVT_SLOT_B, 0x0)
    LOMA(CHAPTER_42)
    FADU(16)
    LOAD2(0x1, UnitDef_Ch16AAlly_12)
    ENUN
    SOUN(SONG_B1)
    TILECHANGE(0x0)
    LOAD2(0x1, UnitDef_Ch16AMixed_1)
    ENUN
    CUMO_CHAR(0xfb)
    STAL(60)
    CURE
    MUSCMID(SONG_SILENT)
    SetBackground(BG_THRONE_NORMAL)
    TEXTSHOW(0xb4a)
    TEXTEND
    FADI(4)
    REMA
    CLEA
    CLEE
    CLEN
    SVAL(EVT_SLOT_B, 0x4000e)
    LOMA(CHAPTER_E_16)
    EvtSetLoadUnitNoREDA // ENOSUPP in EAstdlib
    LOAD2(0x1, UnitDef_Ch16AAlly_0)
    ENUN
    FADU(4)
    CUMO_CHAR(CHARACTER_EPHRAIM)
    STAL(60)
    CURE
    MUSC(SONG_VICTORY)
    TEXTSTART
    TEXTSHOW(0xb4b)
    TEXTEND
    REMA
    LOAD2(0x1, UnitDef_Ch16AAlly_0)
    ENUN
    CUMO_CHAR(CHARACTER_SETH)
    STAL(60)
    CURE
    TEXTSTART
    TEXTSHOW(0xb4c)
    TEXTEND
    REMA
    MOVE(0x0, CHARACTER_EIRIKA, 12, 11)
    MOVE(0x0, CHARACTER_EPHRAIM, 14, 11)
    MOVE(0x0, CHARACTER_SETH, 13, 11)
    STAL2(20)
    MUSCMID(SONG_SILENT)
    FADI(16)
    ENUN
    CLEA
    SVAL(EVT_SLOT_B, 0x0)
    LOMA(CHAPTER_43)
    LOAD2(0x1, UnitDef_Ch16AAlly_14)
    ENUN
    MUSC(SONG_95)
    FADU(16)
    LOAD2(0x1, UnitDef_Ch16AAlly_13)
    SOLOTEXTBOXSTART
    SVAL(EVT_SLOT_B, 0x400008)
    TEXTSHOW(0xb4d)
    TEXTEND
    REMA
    SVAL(EVT_SLOT_B, 0x580030)
    TEXTSHOW(0xb4e)
    TEXTEND
    REMA
    SVAL(EVT_SLOT_B, 0x480060)
    TEXTSHOW(0xb4f)
    TEXTEND
    REMA
    ENUN
    CUMO_CHAR(CHARACTER_EPHRAIM)
    STAL(60)
    CURE
    SetBackground(BG_CASTLE_INTERIOR)
    TEXTSHOW(0xb50)
    TEXTEND
    REMA
    MUSCMID(SONG_SILENT)
    FADI(16)
    CLEA
    CLEE
    CLEN
    STAL(60)
    SVAL(EVT_SLOT_B, 0x4000e)
    LOMA(CHAPTER_E_16)
    EvtSetLoadUnitNoREDA // ENOSUPP in EAstdlib
    LOAD2(0x1, UnitDef_Ch16AAlly_1)
    ENUN
    FADU(16)
    CUMO_CHAR(CHARACTER_SETH)
    STAL(60)
    CURE
    TEXTSTART
    TEXTSHOW(0xb51)
    TEXTEND
    REMA
    STARTFADE
    EvtColorFadeSetup(0x6, 0xa, 4, 128, 128, 128) // ENOSUPP in EAstdlib
    ASMC2(sub_8080014)
    SVAL(EVT_SLOT_B, 0x3000d)
    TILECHANGE(0xffff)
    SVAL(EVT_SLOT_B, 0x3000e)
    TILECHANGE(0xffff)
    EvtColorFadeSetup(0x6, 0xa, 4, 256, 256, 256) // ENOSUPP in EAstdlib
    EVBIT_T(6)
    LOAD2(0x1, UnitDef_Ch16AAlly_1)
    STAL(20)
    FADI(4)
    ENUN
    EVBIT_F(6)
    CLEA
    SVAL(EVT_SLOT_B, 0x0)
    LOMA(CHAPTER_47)
    FADU(16)
    LOAD2(0x1, UnitDef_Ch16AAlly_15)
    ENUN
    CUMO_CHAR(CHARACTER_EPHRAIM)
    STAL(60)
    CURE
    SetBackground(BG_RENAIS_CHAMBER)
    MUSCSLOW(SONG_LEGACY)
    TEXTSHOW(0xb52)
    TEXTEND
    MUSCSLOW(SONG_SILENT)
    FADI(4)
    REMA
    CLEAN
    FADU(4)
    CUMO_CHAR(CHARACTER_EPHRAIM)
    STAL(60)
    CURE
    SetBackground(BG_RENAIS_CHAMBER)
    TEXTSHOW(0xb53)
    TEXTEND
    SOUN(SONG_2EB)
    FAWI(4)
    REMA
    SVAL(EVT_SLOT_2, 0x2e)
    CALL(EventScr_Ch16A_1)
    MUSCSLOW(SONG_LEGACY)
    REMOVEPORTRAITS
    TEXTSHOW(0xb54)
    TEXTEND
    REMA
    FADI(16)
    CLEAN
    FADU(4)
    LOAD2(0x1, UnitDef_Ch16AAlly_16)
    ENUN
    SOUN(SONG_B1)
    TILECHANGE(0x0)
    LOAD2(0x1, UnitDef_Ch16AAlly_17)
    ENUN
    SOUN(SONG_B1)
    TILECHANGE(0x1)
    LOAD2(0x1, UnitDef_Ch16AAlly_18)
    ENUN
    CUMO_CHAR(CHARACTER_SETH)
    STAL(60)
    CURE
    SetBackground(BG_RENAIS_CHAMBER)
    TEXTSHOW(0xb55)
    TEXTEND
    REMA
    CALL(EventScr_RemoveBGIfNeeded)
    SVAL(EVT_SLOT_3, 0x92)
    GIVEITEMTO(CHARACTER_EPHRAIM)
    TEXTSHOW(0xb56)
    TEXTEND
    REMA
    CALL(EventScr_RemoveBGIfNeeded)
    SVAL(EVT_SLOT_3, 0x85)
    GIVEITEMTO(CHARACTER_EIRIKA)
    CHECK_MODE
    SVAL(EVT_SLOT_1, 0x2)
    BNE(0x0, EVT_SLOT_C, EVT_SLOT_1)
    TEXTSHOW(0xb57)
    TEXTEND
    GOTO(0x1)
LABEL(0x0)
    TEXTSHOW(0xb58)
    TEXTEND
LABEL(0x1)
    FADI(16)
    REMA
    CLEA
    CLEE
    CLEN
    SVAL(EVT_SLOT_B, 0x4000e)
    LOMA(CHAPTER_E_16)
    SVAL(EVT_SLOT_B, 0x3000d)
    TILECHANGE(0xffff)
    SVAL(EVT_SLOT_B, 0x3000e)
    TILECHANGE(0xffff)
    LOAD2(0x1, UnitDef_Ch16AAlly_2)
    ENUN
    FADU(16)
    LOAD2(0x1, UnitDef_Ch16AAlly_3)
    ENUN
    SVAL(EVT_SLOT_B, 0x3000d)
    TILEREVERT(0xffff)
    SVAL(EVT_SLOT_B, 0x3000e)
    TILEREVERT(0xffff)
    CUMO_CHAR(CHARACTER_LARACHEL)
    STAL(60)
    CURE
    MUSC(SONG_VICTORY)
    CHECK_MODE
    SVAL(EVT_SLOT_1, 0x2)
    BNE(0xa, EVT_SLOT_C, EVT_SLOT_1)
    TEXTSTART
    TEXTSHOW(0xb59)
    TEXTEND
    REMA
    GOTO(0xb)
LABEL(0xa)
    TEXTSTART
    TEXTSHOW(0xb5a)
    TEXTEND
    REMA
LABEL(0xb)
    LOAD2(0x1, UnitDef_Ch16AAlly_4)
    ENUN
    FADI(16)
    ENUN
    ENDA
};

CONST_DATA EventListScr EventScr_Ch16A_1[] = {
    SADD(EVT_SLOT_A, EVT_SLOT_2, EVT_SLOT_0)
    BACG(BG_BLANK)
    STARTFADE
    EvtColorFadeSetup(0x6, 0xa, 0, 512, 512, 512) // ENOSUPP in EAstdlib
    FAWU(128)
    CALL(EventScr_RemoveBGIfNeeded)
    EVBIT_MODIFY(0x4)
    CALL(EventScr_Ch16A_2)
    MUSCMID(SONG_SILENT)
    REMOVEPORTRAITS
    SADD(EVT_SLOT_2, EVT_SLOT_A, EVT_SLOT_0)
    BACG(0xffff)
    FAWU(4)
    EVBIT_MODIFY(0x0)
    ENDA
};

CONST_DATA EventListScr EventScr_Ch16A_2[] = {
    CHECK_MODE
    SVAL(EVT_SLOT_1, 0x2)
    BNE(0x0, EVT_SLOT_C, EVT_SLOT_1)
    TUTORIALTEXTBOXSTART
    SVAL(EVT_SLOT_B, 0xffffffff)
    TEXTSHOW(0xb5b)
    TEXTEND
    REMA
    CALL(EventScr_Ch16A_3)
    CALL(EventScr_Ch16A_4)
    GOTO(0x1)
LABEL(0x0)
    TUTORIALTEXTBOXSTART
    SVAL(EVT_SLOT_B, 0xffffffff)
    TEXTSHOW(0xb5c)
    TEXTEND
    REMA
    CALL(EventScr_Ch16A_4)
    CALL(EventScr_Ch16A_3)
LABEL(0x1)
    FAWI(16)
    ENDA
};

CONST_DATA EventListScr EventScr_Ch16A_3[] = {
    TUTORIALTEXTBOXSTART
    SVAL(EVT_SLOT_B, 0xffffffff)
    TEXTSHOW(0xb5d)
    TEXTEND
    REMA
    SVAL(EVT_SLOT_7, 0x1)
    BNE(0x0, EVT_SLOT_C, EVT_SLOT_7)
    FADI(16)
    CALL(EventScr_Ch16A_13)
    PROM(CHARACTER_EIRIKA, 0x4, 0x98)
    EVBIT_MODIFY(0x4)
    REMOVEPORTRAITS
    BACG(BG_BLANK)
    STARTFADE
    EvtColorFadeSetup(0x6, 0xa, 0, 512, 512, 512) // ENOSUPP in EAstdlib
    FADU(16)
    GOTO(0x1)
LABEL(0x0)
    CALL(EventScr_Ch16A_5)
LABEL(0x1)
    ENDA
};

CONST_DATA EventListScr EventScr_Ch16A_4[] = {
    TUTORIALTEXTBOXSTART
    SVAL(EVT_SLOT_B, 0xffffffff)
    TEXTSHOW(0xb5e)
    TEXTEND
    REMA
    SVAL(EVT_SLOT_7, 0x1)
    BNE(0x0, EVT_SLOT_C, EVT_SLOT_7)
    FADI(16)
    CALL(EventScr_Ch16A_13)
    PROM(CHARACTER_EPHRAIM, 0x3, 0x99)
    EVBIT_MODIFY(0x4)
    REMOVEPORTRAITS
    BACG(BG_BLANK)
    STARTFADE
    EvtColorFadeSetup(0x6, 0xa, 0, 512, 512, 512) // ENOSUPP in EAstdlib
    FADU(16)
    GOTO(0x1)
LABEL(0x0)
    CALL(EventScr_Ch16A_6)
LABEL(0x1)
    ENDA
};

CONST_DATA EventListScr EventScr_Ch16A_5[] = {
    CALL(EventScr_RemoveBGIfNeeded)
    SVAL(EVT_SLOT_3, 0x98)
    GIVEITEMTO(CHARACTER_EIRIKA)
    EVBIT_MODIFY(0x4)
    TUTORIALTEXTBOXSTART
    SVAL(EVT_SLOT_B, 0xffffffff)
    TEXTSHOW(0xb5f)
    TEXTEND
    REMA
    ENDA
};

CONST_DATA EventListScr EventScr_Ch16A_6[] = {
    CALL(EventScr_RemoveBGIfNeeded)
    SVAL(EVT_SLOT_3, 0x99)
    GIVEITEMTO(CHARACTER_EPHRAIM)
    EVBIT_MODIFY(0x4)
    TUTORIALTEXTBOXSTART
    SVAL(EVT_SLOT_B, 0xffffffff)
    TEXTSHOW(0xb60)
    TEXTEND
    REMA
    ENDA
};

CONST_DATA EventListScr EventScr_Ch16A_7[] = {
    SVAL(EVT_SLOT_2, UnitDef_Ch16AEnemy_6)
    CALL(EventScr_LoadReinforce)
    EVBIT_T(7)
    ENDA
};

CONST_DATA EventListScr EventScr_Ch16A_8[] = {
    SVAL(EVT_SLOT_2, 0x0)
    CALL(EventScr_UnTriggerIfNotFaction)
    SVAL(EVT_SLOT_1, 0x10000)
    CHAI(0x8f)
    SVAL(EVT_SLOT_1, 0x10303)
    CHAI(0x90)
    ENUF(12)
    EVBIT_T(7)
    ENDA
};

CONST_DATA EventListScr EventScr_Ch16A_9[] = {
    SVAL(EVT_SLOT_2, UnitDef_Ch16AEnemy_2)
    CALL(EventScr_LoadReinforce)
    SVAL(EVT_SLOT_2, UnitDef_Ch16AEnemy_3)
    CALL(EventScr_LoadReinforce)
    CAMERA(19, 27)
    SVAL(EVT_SLOT_2, UnitDef_Ch16AEnemy_4)
    CALL(EventScr_LoadReinforce)
    SVAL(EVT_SLOT_2, UnitDef_Ch16AEnemy_5)
    CALL(EventScr_LoadReinforce)
    EVBIT_T(7)
    ENDA
};

CONST_DATA EventListScr EventScr_Ch16A_10[] = {
    SVAL(EVT_SLOT_2, UnitDef_Ch16AEnemy_7)
    CALL(EventScr_LoadReinforceHardMode)
    EVBIT_T(7)
    ENDA
};
