#include "global.h"

u8 Img_MapBattleInfoBox[] = INCBIN_U8("graphics/misc/Img_MapBattleInfoBox.4bpp.lz");

u16 Pal_MapBattleInfoBlue[] = INCBIN_U16("graphics/misc/Pal_MapBattleInfoBlue.gbapal");

u16 Pal_MapBattleInfoRed[] = INCBIN_U16("graphics/misc/Pal_MapBattleInfoRed.gbapal");

u16 Pal_MapBattleInfoGreen[] = INCBIN_U16("graphics/misc/Pal_MapBattleInfoGreen.gbapal");

u16 Pal_MapBattleInfoPurple[] = INCBIN_U16("graphics/misc/Pal_MapBattleInfoPurple.gbapal");

u8 Tsa_MapBattleBoxGfx1[] = INCBIN_U8("graphics/misc/Tsa_MapBattleBoxGfx1.tsa.bin.lz");

u8 Tsa_MapBattleBoxGfx2[] = INCBIN_U8("graphics/misc/Tsa_MapBattleBoxGfx2.tsa.bin.lz");

u8 Tsa_MapBattleBoxGfx3[] = INCBIN_U8("graphics/misc/Tsa_MapBattleBoxGfx3.tsa.bin.lz");

u8 Img_MapBattleInfoHpBar[] = INCBIN_U8("graphics/misc/Img_MapBattleInfoHpBar.4bpp.lz");

u16 Img_MapBattleInfoNum[] = INCBIN_U16("graphics/misc/Img_MapBattleInfoNum.4bpp.lz");

u8 Tsa_ShopWindows[] = INCBIN_U8("graphics/misc/Tsa_ShopWindows.tsa.bin.lz");

u16 Img_ShopGoldBox[] = INCBIN_U16("graphics/misc/Img_ShopGoldBox.4bpp.lz");

u16 CONST_DATA Img_WallBreakAnim[] = INCBIN_U16("graphics/misc/Img_WallBreakAnim.4bpp.lz");

u16 CONST_DATA Pal_WallBreakAnim[] = INCBIN_U16("graphics/misc/Pal_WallBreakAnim.gbapal");

const u8 CONST_DATA Img_GmapSoguSprites[] = INCBIN_U8("graphics/misc/Img_GmapSoguSprites.4bpp.lz");

u16 CONST_DATA Img_PoisonAnim[] = INCBIN_U16("graphics/misc/Img_PoisonAnim.4bpp.lz");

u16 CONST_DATA Pal_PoisonAnim[] = INCBIN_U16("graphics/misc/Pal_PoisonAnim.gbapal");

u16 CONST_DATA Img_LinkArenaWarpFx[] = INCBIN_U16("graphics/misc/Img_LinkArenaWarpFx.4bpp.lz");

u16 CONST_DATA Pal_LinkArenaWarpFx[] = INCBIN_U16("graphics/misc/Pal_LinkArenaWarpFx.agbpal");

u16 CONST_DATA gMapanimBattleinfo_0[] = INCBIN_U16("graphics/misc/gUnknown_089AE4A4.4bpp");

u8 Img_MapAnimBerserkfx_1[] = INCBIN_U8("graphics/misc/Img_089AE7A4.4bpp.lz");

u16 Pal_MapAnimBerserkfx_0[] = INCBIN_U16("graphics/misc/Pal_089AE7C4.agbpal");

u8 CONST_DATA Img_MapAnimAntitoxinPureWater[] = INCBIN_U8("graphics/misc/Img_MapAnimAntitoxinPureWater.4bpp.lz");

u16 CONST_DATA gMapanimBattleinfo_1[] = INCBIN_U16("graphics/misc/gUnknown_089AF310.4bpp");

u16 CONST_DATA Pal_MapAnimPureWater[] = INCBIN_U16("graphics/misc/Pal_MapAnimPureWater.agbpal");

u16 CONST_DATA Pal_MapAnimAntitoxin[] = INCBIN_U16("graphics/misc/Pal_MapAnimAntitoxin.agbpal");

u8 CONST_DATA gMapanimBattleinfo_2[] = INCBIN_U8("graphics/misc/gUnknown_089AF950.4bpp.lz");

u8 CONST_DATA gMapanimBattleinfo_3[] = INCBIN_U8("graphics/misc/gUnknown_089AFAC4.4bpp.lz");

u8 CONST_DATA gMapanimBattleinfo_4[] = INCBIN_U8("graphics/misc/gUnknown_089AFCBC.4bpp.lz");

u16 CONST_DATA Pal_MapAnimBerserkfx_1[] = INCBIN_U16("graphics/misc/Pal_089AFF78.agbpal");

u16 CONST_DATA Pal_MapAnimBerserkfx_2[] = INCBIN_U16("graphics/misc/Pal_089AFFB8.gbapal");

u8 CONST_DATA gMapanimBattleinfo_5[] = INCBIN_U8("graphics/misc/gUnknown_089AFFF8.4bpp.lz");

u8 CONST_DATA Img_MapAnimTorchfx[] = INCBIN_U8("graphics/misc/Img_MapAnimTorchfx.4bpp.lz");

u16 CONST_DATA Pal_MapAnimTorchfx[] = INCBIN_U16("graphics/misc/Pal_MapAnimTorchfx.agbpal");

u8 Img_MapAnimUnlockObjfx[] = INCBIN_U8("graphics/misc/Img_MapAnimUnlockObjfx.4bpp.lz");

u16 Pal_MapAnimUnlockObjfx[] = INCBIN_U16("graphics/misc/Pal_MapAnimUnlockObjfx.agbpal");

u8 Img_MapAnimUnlockBgfx[] = INCBIN_U8("graphics/misc/Img_MapAnimUnlockBgfx.4bpp.lz");

u16 Pal_MapAnimUnlockObjfx_Unk[] = INCBIN_U16("graphics/misc/Pal_MapAnimUnlockObjfx_Unk.agbpal");

u8 Tsa_MapAnimUnlockBgfx[] = INCBIN_U8("graphics/misc/Tsa_MapAnimUnlockBgfx.tsa.bin");

u8 Tsa_MapAnim_0[] = INCBIN_U8("graphics/misc/Tsa_089B0864.tsa.bin");

u8 Img_MapAnimRepairfx[] = INCBIN_U8("graphics/misc/Img_MapAnimRepairfx.4bpp.lz");

u8 Tsa_MapAnim_1[] = INCBIN_U8("graphics/misc/Tsa_089B17A4.tsa.bin");

u16 ApConf_MapAnimSleep[] = INCBIN_U16("graphics/misc/ApConf_MapAnimSleep.4bpp");

u8 Img_MapAnimBerserkfx[] = INCBIN_U8("graphics/misc/Img_MapAnimBerserkfx.4bpp.lz");

u16 Pal_MapAnimBerserkfx[] = INCBIN_U16("graphics/misc/Pal_MapAnimBerserkfx.gbapal");
