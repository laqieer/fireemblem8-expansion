...
- A supported modern **release** build (`MODERN_CONFIG=release`, `-DNDEBUG`,
  ...
  `DebugToolsLauncher_*`/selector symbols) and by equivalent host-compiled checks
  (`test_registry_disabled_path_behavior_and_symbol_omission`).
- The archival agbcc lane defines `FE8_ARCHIVAL_BUILD`; all supported
  debugtools implementation units, including the selector and cursor editor,
  compile empty. Issues #123 and #125 add no archival behavior or byte-match
  requirement.
