...
extension](ownership-probe-producers.md) for #225 and the
[dependency-only host compiler](ownership-probe-dependencies.md) for #228.
D depends on P, not on the view selector. Optional runtime inputs remain
separately allocated to #227.
...
and `Snapshot(reuse=...)`. The separate D extension adds `Command.dependency_only`
and actual driver/cc1 execution receipts through P's producer contract, without
requiring the view selector. Optional `runtime_files` remains a separate
#227 contract.
