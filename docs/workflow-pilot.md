...
The launcher executes this producer through the same exact-Git sealed boundary
as event classification. Its full metadata/helper closure is required from the
trusted commit; restored working files cannot satisfy missing committed sources.
Runner event-file/output transport remains outside the worker, with both
transport modules loaded from their sealed bytes. Metadata validation and digest
inputs remain #199's unchanged contract. The only explicit dynamic stdlib
addition is `time`, required by metadata timestamp formatting through `datetime`;
there is no generic import allowance or legacy proof fallback.
...
policy. Its production consumers are the isolated launcher's closed
`classify-event` and `attest-metadata-event` modes. The event decisions, output keys, Build graph, required
...
The metadata-event mode similarly executes `metadata_event.capsule_main` with
the complete `pr_metadata`, `candidate_evidence` and event-loader source closure.
Only its pure transition digest runs in the worker; the injected CLI transport
retains #199's arguments and `digest=<sha256>` output. Missing committed helpers
or an undeclared lazy dependency reject without emitting attribution.
`pr-metadata` remains its existing separately authorized operator mode. This
integration does not resolve or narrow the module-capability design hold above.
