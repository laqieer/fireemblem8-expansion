...
of the affected data object. Because some referenced assets (for example
`.4bpp.lz` graphics) are themselves generated by the legacy build's own
compression rules and are not committed to the repository, run a normal
`make` (or `make fireemblem8.gba`) at least once before `expansion-modern-all`
so those assets exist on disk; this target does not generate them itself.
