...
| `EXPANSION_CUSTOM_SPELL_EFFECTS` | `0` or `1` | `0` | issues #77/#78 optional typed custom battle spell runtime and strict package adapter (fingerprint); `1` requires a selected `ASSET_MANIFEST` with at least one valid `custom-spell-effect` record, whose generated descriptor, `SpellAssoc` binding, and assets bind ABI 1 plus inventory/resource digests, while the catalog-free disabled identity remains unchanged |
...
| `features.custom_spell_effects`, enabled `custom_spell_effect_contract.runtime_abi`, `inventory_digest`, and `resource_budget_digest` | issues #77/#78 default-off custom battle spell runtime and adapter; enabled identity binds ABI v1 plus the selected source manifest's canonical generated descriptors, compressed frame assets, sound/OAM timing, binding inventory, and actual resource requirements. Disabled metadata records zero ABI and SHA-256-of-empty inventory/resource digests without adding that contract tuple to the fingerprint, so the default fingerprint remains unchanged. Diagnostic identity only -- see `docs/custom_spell_effects.md`; it does not touch the save format |
