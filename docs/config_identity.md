...
| `EXPANSION_CUSTOM_SPELL_EFFECTS` | `0` or `1` | `0` | issues #77/#78 optional typed custom battle spell runtime and strict package adapter (fingerprint); enabled identity binds ABI 1 plus the selected `ASSET_MANIFEST` generated inventory/resource digests, while the catalog-free disabled identity remains unchanged |
