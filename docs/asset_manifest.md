...
reject. Its logical source digest uses those stable identities, not source
materialization mtimes. Independent captures of identical inputs therefore
produce identical generated bytes; real content, mode or membership changes
still change the digest. No source timestamps or other metadata are normalized.
