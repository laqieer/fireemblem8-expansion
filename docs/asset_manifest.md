...
relative_output, content = manifest.render_discovery_artifact(
    ...
    source_identities=captured_path_mode_sha256)
```

The artifact renderer requires complete `(path, octal_mode, sha256)` identities
for the manifest itself and every discovery source, supplied by the existing
capture authority. It verifies each actual opened regular file's mode/content
and revalidates the descriptor; missing, extra, duplicate or mismatched claims
reject. The manifest records are parsed from those verified manifest bytes, not
from a separate path read, so the rendered consumer groups and digest share one
attested source identity. Its logical source digest uses those stable
identities, not source materialization mtimes. Independent captures of
identical inputs therefore produce identical generated bytes; real content,
mode or membership changes still change the digest. No source timestamps or
other metadata are normalized.

Ordinary `render_discovery_makefile` and CLI discovery retain their existing
filesystem-mtime stamp. Their digest intentionally differs from the captured
artifact's identity digest; both retain the same validated consumer groups
and ordinary path/failure behavior. A captured identity does not grant access
to an undeclared file or replace the executor's actual consumption checks.
