...
3. Run unreachable-producer, replacement and nested publication controls.
   ...
   final/nested generated bytes rather than inferring nonexecution from an
...
and expires only when the caller releases it. Active publication/observation
owners retain every required byte and receipt. Pure-reader/native-artifact
