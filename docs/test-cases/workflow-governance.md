...
3. Repeat with a nonempty essential-contract reason, keep the same full run
   ...
   The producer's launcher must use the complete exact-Git sealed source
   closure. Replace working metadata/runtime/helper files after committing the
   owned fixture and require the same digest. Remove each committed helper
...
2. Run the existing stdlib runner:
   ...
   Run the merged metadata-event source-closure integration:
   `/usr/bin/python3 -m unittest scripts.workflow_pilot.tests.test_metadata_capsule -v`.
