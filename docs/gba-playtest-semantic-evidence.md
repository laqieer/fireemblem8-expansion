...
| `test_probe_bindings.py` (1) | Fake `nm` executables plus `make -n` of every debug/release hook and QoL binding exercise explicit/configured forwarding and failure resolution. | Every emitted production probe command, including debug/release danger-overlay positive and negative pairs, forwards `MODERN_NM`; explicit/configured tools resolve a symbol. | Missing tool reports `ProbeBindingError`. |
