...
| `tools/gba-playtest/tests/test_expansion_language_menu.py::ExpansionLocaleVanillaIsolationTests.test_owned_runtime_files_have_no_vanilla_language_or_xmap_calls` | Modern objects reject vanilla language relocations; the host formatter/initializer varies and preserves `gLanguageMode` and XMAP state. This also covers macro-only XMAP coupling that `nm` alone cannot represent. |
