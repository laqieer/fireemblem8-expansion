...
Japanese (`ja`), Simplified Chinese (`zh-Hans`), French (`fr`), German (`de`),
Spanish (`es`), and Italian (`it`) localization. Verified FE8U-target
decisions feed the opt-in modern game catalog; raw-address values remain
import provenance only and never become runtime keys.

The European source under `texts/locales/eu/` is regenerated by
`scripts/localization/eu.py` from authorized FE8EU ROM SHA-256
`80f94bf10da412e6d8d1ba11c043107f4873bc17fecceb02e6a7da3d1a261d6d`.
Its shared Huffman tree contains five native banks in `en,de,fr,es,it` order.
`texts/locales/mapping/fe8eu_to_fe8u.json` maps all 3,414 FE8U targets with
official indexed providers, reviewed FE8U-target translations, explicit
regional split-message concatenations, and zero fallback.
...
European static resources use `graphics/localized_ui/eu/manifest.json`; the
shared extracted Latin font uses `graphics/fonts/eu/manifest.json`. Both pin
source addresses, sizes, hashes, and round-trip PNG/binary artifacts.
