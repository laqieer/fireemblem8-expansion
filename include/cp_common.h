...
// ??? AiTryDoCombatInRangeFromPosition(???);
// ??? AiIsUnitAtPositionDifferentAllegiance(???);
s8 AiFunc_CountEnemiesInRange(const void*);
// ??? AiTryAttackFromConfiguredPosition(???);
s8 AiFunc_StoreAiCounter(const void*);
s8 AiFunc_RestartDecisionState(const void*);
// ??? AiFunc_RestartDecisionStateDuplicate(???);
// ??? AiTryMoveToSpecificPosition(???);
s8 AiCountEnemyInRangeOrTryMoveToSpecificPosition(const void*);
s8 AiFunc_CheckUnitInBoundingBox(const void*);
// ??? AiDecideTalkBetweenChars(???);
s8 AiFunc_FindLeaderCombatTarget(const void*);
// ??? AiIsUnitEnemyWithStoredCharId(???);
// ??? AiFunc_AttackUnitWithCharId(???);
// ??? AiIsUnitEnemyWithStoredUnitId(???);
// ??? AiIsUnitEnemyInDirectionOfTarget(???);
s8 AiFunc_AttackStoredTargetOrMoveToward(const void*);
s8 AiFunc_TryHealStaffByChance(const void*);
s8 AiFunc_TrySpecialItemsByChance(const void*);
s8 AiBallistaRideExit(const void * input);
s8 AiFunc_MoveTowardsInputPosition(const void*);
// ??? AiFunc_EndDecision(???);
// ??? GetAiStaffFuncIndex(???);
...
// ??? AiStaffWarp(???);
// ??? AiStaffRestore(???);
// ??? AiUnitHasUsableWeaponOrStaff(???);
// ??? GetAiSilenceEffectivenessScore(???);
// ??? AiStaffSilence(???);
// ??? AiStaffSleepBerserk(???);
// ??? AiStaffBarrier(???);
s8 GetAiWarpDestinationNearThreatenedAlly(struct Vec2*);
...
// ??? AiGetRangeValueAt(???);
// ??? AiFindReachableDoorPosition(???);
// ??? AiFindClosestChestPosition(???);
// ??? AiFindReachableUnlockPosition(???);
// ??? AiTryDoRogueSpecialItems(???);
// ??? AiSetMovCostTableWithPassableWalls(???);
// ??? AiSetMovCostTableWithPassableTerrain(???);
// ??? AiSetMovCostTableWithTwoPassableTerrains(???);
void InitAiMoveMapForUnit(struct Unit*);
// ??? GenerateUnitExtendedMovementMapOnRange(???);
// ??? GenerateUnitExtendedMovementMapNeglectWall(???);
// ??? GenerateExtendedMovementMapNeglectWall(???);
void GenerateExtendedMovementMapOnRangeNeglectWall(int x, int y, const s8* cost); // AiMapRangeFillMovementWithPassableWalls
void GenerateUnitExtendedMovementMapOnRangeNeglectWall(int, int, struct Unit*);
// ??? GenerateUnitExtendedMovementMapOnRangeNeglectDoor(???);
// ??? GenerateExtendedMovementMapOnRangeNeglectDoor(???);
// ??? GenerateUnitExtendedMovementMapOnRangeNeglectWallSnag(???);
...
void GenerateUnitExtendedMovementMapOnRangeAt(int, int, struct Unit*);
