...
// ??? ClassChgSel_InitDescAndBg(???);
// ??? LoadBattleSpritesForBranchScreen(???);
// ??? ClassChgSel_OnEndCleanupBm(???);
// ??? PrepClassChgOnCancel(???);
// ??? ClassChgSel_RoutePreEndByType(???);
// ??? ClassChgSel_RouteOnEndByType(???);
// ??? StartAndWaitPromoSelect(???);
// ??? PromoMain_WaitSelectDone(???);
void ClassChgSel_SetBlendWindowConfig(void);
void ClassChgSel_SetupBattleTerrain(u32, s16, s16);
void ClassChgSel_StartClassBattleSprite(int, int, int, int, int);
u8 LoadClassBattleSprite(s16*, u16, u16);
// ??? NewCcramifyEnd(???);
// ??? RefreshBMapDisplay_FromPromotion(???);
