...
| `combat.json` (issue #13) | The selector's default Chapter 4 target reaches the chapter's own scripted `FIGHT`, resolved by the REAL battle engine (`Event3F_ScriptBattle`, `EV_CMD_SCRIPT_BATTLE`): the target enemy `gUnitArrayRed[0]` is alive at full HP (`maxHP` `0x0202eba6` and `curHP` `0x0202eba7` both `15`), `curHP` transitions `15 -> 0` at the resolving SCRIPT_BATTLE frame while `maxHP` stays `15`, then `pCharacterData` (`0x0202eb94`) is cleared to a null `0x00000000` (death) -- relocation-independent semantic HP scalars and a null-field marker, never a nonzero pointer, framebuffer, or timing | debug only (selector-based Chapter 4 boot) |
