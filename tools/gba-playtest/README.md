...
| `run_map_menu_presentation_checks.py` generated scenario (issue #168) | `TC-MAP-MENU-168`: the named all-locales/all-features release captures the localized Danger first row and complete help, drives the overlay lifecycle, cancels, and proves map interactivity; host coverage owns all seven locale outputs and every enabled/disabled command composition | release |
