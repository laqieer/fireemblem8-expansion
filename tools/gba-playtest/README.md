...
| `run_blue_phase_delegate_checks.py` generated scenarios (issues #87/#168) | `TC-AUTOPLAY-CHARGE-001`: the enabled debug ROM uses the real localized Charge first row, captures complete R-help, delegates the current blue phase, and reaches the next interactive blue phase with PLAYER restored; matching default debug/release ROMs retain zero blue AI actions | debug (positive + negative) / release (negative) |
| `run_map_menu_presentation_checks.py` generated scenarios (issue #168) | `TC-MAP-MENU-168`: the named all-locales/all-features release captures the localized Danger first row and complete help, drives the overlay lifecycle, cancels, and proves map interactivity; a focused debug profile reaches an unlocked-Guide skirmish and pins all nine rows, bounded frame/End geometry, and the bottom-screen framebuffer region; host coverage owns all seven locale outputs, feature compositions, nine-row skirmish/dungeon geometry, and shorter/story controls | release + debug |
