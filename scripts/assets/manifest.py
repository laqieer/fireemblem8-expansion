...
def render_discovery_makefile(records):
    ...
    return _render_discovery_makefile(records, hashlib.sha256(source_stamp).hexdigest())


def _render_discovery_makefile(records, source_digest):
    groups = (
...
def _captured_source_digest(manifest_path, records, identities):
    ...
    for path in sorted(expected):
        ...
        if (
            ...
            or not stat.S_ISREG(before.st_mode) or supplied[path] != (mode, digest)
        ):
