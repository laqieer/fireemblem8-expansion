...
def capsule_main(request: dict, context) -> str:
    return event_digest(**request)


def main(argv: list[str] | None = None, *, attestor=None, event_loader=None) -> int:
    if event_loader is None:
        from . import event_classifier

        event_loader = event_classifier.load_event
    if attestor is None:
        attestor = event_digest
    ...
    try:
        digest = attestor(
            payload=event_loader(args.event_path),
