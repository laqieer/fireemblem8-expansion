...
from .publisher_inventory import (
    ...
    WORKFLOW_PATH, control_header, normalize_invocation,
)
...
def inventory() -> Inventory:
    ...
    main("fail", "exit 1", Resource.PROCESS, Access.WRITE, count=2, placements=(
        Placement(case("host-temp-boundary", 1)), Placement(runtime_rejection),
    ))
    main("transport-fail", "exit 125", Resource.PROCESS, Access.WRITE, count=2, placements=(
    ...
    main("candidate-exit", 'exit "$candidate_status"', Resource.PROCESS, Access.WRITE, context=(Context("if", "builder_main.candidate-result", "success"),))
    main(
        "legacy-membership",
