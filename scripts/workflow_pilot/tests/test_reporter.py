...
class FailClosedDataTests(unittest.TestCase):
    ...
    def test_post_review_override_mutation_is_rejected(self):
        decisions = minimal_decisions()
        decisions["pull_requests"][0]["threshold"]["override_history"] = [
            {
                "recorded_at": "2026-01-01T05:00:00Z",
                "enabled": True,
                "reason": "too late",
            }
        ]
        self.assert_rejected(
            decisions=decisions,
            pattern="recorded after first review",
        )
...
class ArtifactLifecycleTests(unittest.TestCase):
    ...
    def test_orphan_duplicate_expired_and_deletion_ready_artifacts_reject(self):
        ...
        self.assert_rejected(
            fixture,
            minimal_decisions(),
            "exactly one executable consumer",
        )
