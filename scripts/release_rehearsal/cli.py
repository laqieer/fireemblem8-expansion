...
def cmd_workflow_guard(args) -> int:
    ...
    `RELEASE_TARGET_SHA` contract in the actual sole release job's
    job-level `env` mapping wherever a release publication-eligibility
    target is invoked; step env, outputs, comments, or unrelated
    expressions cannot satisfy it) is folded in here too, rather than
