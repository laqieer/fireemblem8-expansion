...
class CombinedCoverageTests(unittest.TestCase):
    ...
    def test_summary_accounts_for_current_fallback_partition(self):
        ...
        self.assertEqual(summary["translated_target_count"], 1608)
        self.assertEqual(summary["explicit_fallback_target_count"], 1806)
        self.assertEqual(
            summary["actionable_not_yet_verified_target_count"], 1794
        )
        self.assertEqual(summary["non_actionable_fallback_target_count"], 12)
        self.assertEqual(
            summary["febuilder_unique_uncontested_candidate_count"], 1382
        )
        self.assertEqual(summary["structural_high_candidate_count"], 27)
        self.assertEqual(summary["structural_reference_candidate_count"], 1354)
        self.assertEqual(summary["combined_candidate_target_count"], 1389)
        self.assertEqual(
            summary["combined_unblocked_candidate_target_count"], 1383
        )
        self.assertEqual(summary["combined_blocked_target_count"], 21)
        self.assertEqual(summary["residual_target_count"], 390)
        self.assertEqual(
    ...
    def test_conflicts_and_collisions_remain_explicit(self):
        ...
        self.assertEqual(summary["febuilder_actionable_collision_count"], 16)
        self.assertEqual(summary["structural_context_collision_count"], 20)
        self.assertEqual(
        ...
        self.assertEqual(
            self.report["intersections"]["candidate_and_blocked_count"], 6
        )
        self.assertTrue(
    ...
    def test_check_command_passes(self):
        ...
        self.assertIn(
            "translated=1608 candidates=1383 blocked=21 residual=390",
            result.stdout,
        )
