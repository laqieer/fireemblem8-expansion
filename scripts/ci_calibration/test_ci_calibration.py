...
class CalibrationControls(unittest.TestCase):
    ...
    def test_workflow_is_push_only_separate_pinned_and_least_privilege(self):
        ...
        self.assertEqual(data["name"], "Issue 180 disposable original-root acceptance 19")
        self.assertEqual(data["on"], {"push": {"branches": [policy.BRANCH]}})
        self.assertEqual(data["permissions"], {"contents": "read"})
        self.assertEqual(data["concurrency"]["group"], "issue180-disposable-diagnostic-baseline-19")
        self.assertEqual(set(data["jobs"]), {"disposable-root-acceptance-19"})
        job = data["jobs"]["disposable-root-acceptance-19"]
        self.assertEqual(job["name"], "Disposable original-root acceptance only 19")
        ...
        self.assertEqual(uploads[0]["with"]["name"],
                         "issue180-ci-baseline-19-${{ github.run_id }}-attempt-${{ github.run_attempt }}")
        uploaded = uploads[0]["with"]["path"].splitlines()
        self.assertEqual(uploaded, [
            "${{ runner.temp }}/issue180-ci-baseline-19-${{ github.run_id }}/" + name
        ...
        self.assertEqual(outputs, ["$RUNNER_TEMP/issue180-ci-baseline-19-$GITHUB_RUN_ID"] * 2)
