...
from scripts.ci_calibration import kernel, observation_failure, policy, supervisor, volume_mount, worker
...
class CalibrationControls(unittest.TestCase):
    ...
    def test_every_pinned_limit_is_classified_without_importing_candidate(self):
        ...
        manifest = policy.profile_manifest(defaults, observation_count=defaults["entries"])
        ...
        self.assertIsNone(manifest["observations"]["original"])
        self.assertEqual(manifest["observations"]["original_effective"], 32768)
        self.assertEqual(manifest["observations"]["diagnostic_effective"], policy.POLICY_SENTINEL)
        with self.assertRaises(policy.GuardError):
            policy.profile_manifest({**defaults, "unknown_limit": 1}, observation_count=32768)
    ...
    def test_profile_construction_is_immutable_and_preserves_one_clock_and_inherited_methods(self):
        ...
        budget, limits, original, manifest = worker.calibration_budget(Limits, ProbeBudget, deadline)
        self.addCleanup(budget.close)
        self.assertEqual(budget.deadline, deadline)
        self.assertEqual(budget.started, deadline - 3600)
        for name in ("run", "charge", "remaining", "plan", "admit_planned_state", "read_bytes", "close"):
            self.assertIs(getattr(type(budget), name), getattr(ProbeBudget, name))
        budget.charge("control", 1)
        self.assertEqual(budget.bytes, {"control": 1})
        ...
        self.assertEqual(limits.entries, 32768)
        self.assertIsNone(original["observations"])
        self.assertEqual(Limits().observation_count, 32768)
        self.assertEqual(limits.observations, policy.POLICY_SENTINEL)
        self.assertEqual(limits.observation_count, policy.POLICY_SENTINEL)
        for name in policy.RETAINED:
            self.assertEqual(getattr(limits, name), original[name])
        for name in policy.RELAXED:
            self.assertEqual(getattr(limits, name), policy.POLICY_SENTINEL)
        with self.assertRaises(dataclasses.FrozenInstanceError):
            limits.pending_bytes = 1
        self.assertEqual(set(manifest), set(original))

    def test_none_and_effective_observations_are_not_zero_unlimited_or_unknown(self):
        from scripts.validation_ownership.budget import Limits, ProbeBudget

        original = dataclasses.asdict(Limits())
        for value in (None, 0, -1, False, True, 32768.0, "32768", policy.POLICY_SENTINEL):
            with self.subTest(effective=value), self.assertRaises(policy.GuardError):
                policy.profile_manifest(original, observation_count=value)
        for change in (
            {"observations": 0}, {"observations": policy.POLICY_SENTINEL},
            {"observations": 32768}, {"unknown": 1},
        ):
            with self.subTest(change=change), self.assertRaises(policy.GuardError):
                policy.profile_manifest({**original, **change}, observation_count=32768)
        missing = {key: value for key, value in original.items() if key != "observations"}
        with self.assertRaises(policy.GuardError):
            policy.profile_manifest(missing, observation_count=32768)

        for value in (None, 0, 32768.0, policy.POLICY_SENTINEL):
            bad = dataclasses.make_dataclass(
                "BadEffectiveCount", [], bases=(Limits,), frozen=True,
                namespace={"observation_count": property(lambda self, value=value: value)},
            )
            with self.subTest(getter=value), self.assertRaises(policy.GuardError):
                worker.calibration_budget(bad, ProbeBudget, time.monotonic() + 3600)
        entries_only = dataclasses.make_dataclass(
            "IgnoresExplicitObservations", [], bases=(Limits,), frozen=True,
            namespace={"observation_count": property(lambda self: self.entries)},
        )
        with self.assertRaisesRegex(policy.GuardError, "explicit cumulative sentinel"):
            worker.calibration_budget(entries_only, ProbeBudget, time.monotonic() + 3600)

    def test_real_diagnostic_profile_retains_fixed_record_and_plan_admission(self):
        from scripts.validation_ownership.budget import (
            Limits, MakeProbeError, MAX_PENDING_RECORD_BYTES, MAX_PLANNED_STATE_BYTES, ProbeBudget,
        )

        self.assertEqual((MAX_PENDING_RECORD_BYTES, MAX_PLANNED_STATE_BYTES), (policy.MIB, policy.MIB))
        for mode in ("record", "plan"):
            with self.subTest(mode=mode):
                budget, limits, _, _ = worker.calibration_budget(
                    Limits, ProbeBudget, time.monotonic() + 3600,
                )
                self.addCleanup(budget.close)
                self.assertEqual(limits.pending_bytes, policy.POLICY_SENTINEL)
                self.assertEqual(limits.observation_count, policy.POLICY_SENTINEL)
                if mode == "record":
    ...
    def test_real_native_profile_keeps_capsule_grant_and_scalar_failure_telemetry(self):
        ...
        from scripts.validation_ownership.tests.test_foundation import ObservationAllowanceTests

        fixture = ObservationAllowanceTests()
        fixture.setUp()
        self.addCleanup(fixture.tearDown)
        names = fixture.sources()
        deadline = time.monotonic() + 3600
        template, limits, original, _ = worker.calibration_budget(Limits, ProbeBudget, deadline)
        self.addCleanup(template.close)
        self.assertEqual(limits.entries, 32768)
        native_budget = type(template)(
            limits=dataclasses.replace(limits, entries=64), original_deadline=deadline,
        )
        self.assertIs(native_budget.charge.__func__, ProbeBudget.charge)
        self.assertIs(native_budget.run.__func__, ProbeBudget.run)
        self.assertEqual(native_budget.deadline, deadline)
        with fixture.session(budget=native_budget) as session:
            observer = observation_failure.Observer(ProbeSession, native_budget)
            for total in (32, 64, 96):
                fixture.read_sources(session, names[:32])
                self.assertEqual(session.observations_used, total)
            try:
                fixture.read_sources(session, names[:33], metadata=True)
            except MakeProbeError as error:
                telemetry = observer.capture(error)
                self.assertEqual(telemetry["status"], "attributed")
                self.assertEqual(telemetry["mode"], "command")
                self.assertEqual(telemetry["observations"], 64)
                self.assertEqual(telemetry["effective_observation_count"], 64)
                self.assertEqual(telemetry["effective_observation_limit"], 16 * policy.MIB)
                self.assertTrue(telemetry["count_predicate"])
                self.assertFalse(telemetry["byte_predicate"])
                self.assertLess(len(policy.encoded(telemetry)), 1024)
            else:
                self.fail("surplus cumulative observations widened the actual capsule grant")
            self.assertEqual(session.observations_used, 160)
            self.assertTrue(native_budget.failed)
            self.assertTrue(native_budget.closed)
        fixture.fixture.assert_clean(session)
        self.assertFalse(native_budget.producer_waiters)
        self.assertEqual(dataclasses.asdict(Limits()), original)
        self.assertEqual(limits.entries, 32768)
        self.assertEqual(limits.observation_count, policy.POLICY_SENTINEL)
    ...
    def test_nested_namespace_cannot_omit_control_write_qualification(self):
        _, data = supervisor.tool([
            "/usr/bin/python3", "-I", "-S", "-B", "-c",
            "import os,sys;sys.path.insert(0,sys.argv[1]);import kernel,policy,worker;"
            "assert os.getuid()!=0;kernel.prctl(38,1);"
            "print(policy.encoded(worker.proc_protection_probe()).decode())",
            str(Path(kernel.__file__).parent),
        ], timeout=5)
        proof = {
            ...
            "proc_protection": json.loads(data),
        }
        ...
        for defect in ("missing", "writable-success", "state-changed", "readonly"):
            changed = copy.deepcopy(proof)
            proc = changed["proc_protection"]
            if defect == "missing":
                del changed["proc_protection"]
            elif defect == "writable-success":
                proc["denied_writes"]["/proc/self/oom_score_adj"]["errno"] = 0
            elif defect == "state-changed":
                proc["denied_writes"]["/proc/self/oom_score_adj"]["after"] = "-1000"
            else:
                proc["proc_mount_flags"] |= 1
            with self.subTest(defect=defect), self.assertRaises(policy.GuardError):
                policy.validate_nested_probe(changed, **arguments)
    ...
    def test_workflow_is_push_only_separate_pinned_and_least_privilege(self):
        ...
        self.assertEqual(data["name"], "Issue 180 disposable original-root acceptance 20")
        self.assertEqual(data["on"], {"push": {"branches": [policy.BRANCH]}})
        self.assertEqual(data["permissions"], {"contents": "read"})
        self.assertEqual(data["concurrency"]["group"], "issue180-disposable-diagnostic-baseline-20")
        self.assertEqual(set(data["jobs"]), {"disposable-root-acceptance-20"})
        job = data["jobs"]["disposable-root-acceptance-20"]
        self.assertEqual(job["name"], "Disposable original-root acceptance only 20")
        ...
        self.assertEqual(uploads[0]["with"]["name"],
                         "issue180-ci-baseline-20-${{ github.run_id }}-attempt-${{ github.run_attempt }}")
        uploaded = uploads[0]["with"]["path"].splitlines()
        self.assertEqual(uploaded, [
            "${{ runner.temp }}/issue180-ci-baseline-20-${{ github.run_id }}/" + name
        ...
        self.assertEqual(outputs, ["$RUNNER_TEMP/issue180-ci-baseline-20-$GITHUB_RUN_ID"] * 2)
