...
class CalibrationControls(unittest.TestCase):
    ...
    def test_workflow_is_push_only_separate_pinned_and_least_privilege(self):
        ...
        self.assertEqual(data["concurrency"]["group"], "issue180-disposable-diagnostic-baseline-5")
        self.assertEqual(set(data["jobs"]), {"disposable-graph-calibration-5"})
        job = data["jobs"]["disposable-graph-calibration-5"]
