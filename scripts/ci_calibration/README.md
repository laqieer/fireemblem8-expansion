# Disposable issue-180 CI baseline 8
...
`calibration/issue-180-ci-baseline-8` by the repository owner. The workflow
...
Baseline 8 selects that corrected immutable CURRENT and the same BASE798.
No resource policy, external envelope, containment guard or worker method
changes from baseline 7; the new first-creation identity does not reopen it.
...
`bc02e0f9a271abf0052f0ce17de1cf29393cc422`, with full Git history and recursively
...
actionlint .github/workflows/issue180-ci-baseline-8.yml
```
...
```bash
git push origin HEAD:refs/heads/calibration/issue-180-ci-baseline-8
```
