# Disposable issue-180 CI baseline 6
...
`calibration/issue-180-ci-baseline-6` by the repository owner. The workflow
...
Baseline 5 qualified all seven hosted controls and invoked the pinned graph
once. It failed with the native aggregate filesystem-observation rejection;
its artifacts do **not** distinguish count from byte exhaustion. Baseline 6
adds only bounded scalar failure telemetry and a fresh first-creation identity.
Every source pin, resource value, formula and containment guard is unchanged.
The previous attempt remains closed; these additions do not retroactively
attribute its failure or authorize another invocation.
...
python3 -m unittest scripts.ci_calibration.test_runtime_identity -v
python3 -m unittest scripts.ci_calibration.test_output_phase -v
python3 -m unittest scripts.ci_calibration.test_observation_failure -v
actionlint .github/workflows/issue180-ci-baseline-6.yml
```
...
The scalar controls call the real narrow observation reservation/counter/grant
methods with tiny inputs and use an explicit traceback/report carrier, not a
full graph or hosted native supervisor. Count-only, byte-only, both, exact
boundaries, mutable grants, malformed/unrelated frames, original errors and
bounded real-pipe serialization are covered. Hosted graph attribution remains
unmeasured until Main separately freezes and triggers the new scope.

After Main reviews the complete commit and envelope, the only trigger is:

```bash
git push origin HEAD:refs/heads/calibration/issue-180-ci-baseline-6
```
