...
class RuntimeIdentityTests(unittest.TestCase):
    ...
    def private_view(self, *, alternatives, nested, proc_readonly=False):
        directory = self.root / f"view-{alternatives}-{nested}-{proc_readonly}"
        directory.mkdir()
        code = r'''
import json,os,sys
from pathlib import Path
sys.path.insert(0,sys.argv[1])
import entry,kernel,policy,runtime_view,worker
...
kernel.mount_private_proc(root/"proc")
if sys.argv[7]=="yes": ns.recursive_attributes(root/"proc",1|2|4|8)
...
if sys.argv[6]=="yes":
    try:
        if sys.argv[7]!="yes": result["proc_protection"]=worker.proc_protection_probe()
        result["nested"]=worker.run_nested_probe(config)
    except policy.GuardError as error: result["nested_failure"]=str(error)
...
'''
    ...
    def test_production_proc_policy_reproduces_readonly_failure_and_preserves_protection(self):
        original = self.private_view(alternatives=True, nested=True, proc_readonly=True)
        self.assertEqual(original["proc_mount_flags"] & 15, 15)
        self.assertIn("uid_map", original["nested_failure"])
        self.assertIn("Read-only file system", original["nested_failure"])
        corrected = self.private_view(alternatives=True, nested=True)
        self.assertEqual(corrected["proc_mount_flags"] & 15, kernel.PROC_MOUNT_FLAGS)
        self.assertNotIn("nested_failure", corrected)
        for proof in (corrected["proc_protection"], corrected["nested"]["proc_protection"]):
            policy.validate_proc_protection(proof)
            self.assertEqual(proof["uid"], 0)
            self.assertEqual(proof["oom_score_adj_after"], "0")
            for result in proof["denied_writes"].values():
                self.assertEqual(result["before"], result["after"])
        self.assertIn("cgroup_control_probe", corrected["nested"])

    def test_actual_nonroot_host_process_cannot_disable_oom_or_write_global_controls(self):
        code = (
            "import os,sys\n"
            "sys.path.insert(0,sys.argv[1])\n"
            "import kernel,policy,worker\n"
            "assert os.getuid()!=0\n"
            "kernel.prctl(38,1)\n"
            "assert int(kernel.status()['CapEff'],16)==0\n"
            "print(policy.encoded(worker.proc_protection_probe()).decode())\n"
        )
        _, output = supervisor.tool([
            "/usr/bin/python3", "-I", "-S", "-B", "-c", code, str(Path(kernel.__file__).parent),
        ], timeout=5)
        observed = json.loads(output)
        policy.validate_proc_protection(observed)
        self.assertEqual(observed["uid"], os.getuid())
        self.assertTrue(observed["denied_writes"]["/proc/self/oom_score_adj"]["opened"])
