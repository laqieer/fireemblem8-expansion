...
class Volume:
    ...
    def prepare(self):
        ...
        self.uncertain_device = False
        self.mounted = True
        tool(["/usr/bin/mount", "-t", "ext4", "-o", "nosuid,nodev", device, str(self.mountpoint)])
    ...
    def close(self):
        ...
        if self.mounted:
            tool(["/usr/bin/umount", str(self.mountpoint)], timeout=15)
            self.mounted = False
...
class Protocol:
    ...
    def feed(self, data):
        ...
        if self.total > self.maximum:
            raise policy.GuardError("external worker output byte bound exceeded")
    ...
    try:
        ...
        with selectors.DefaultSelector() as selector:
            ...
            while selector.get_map():
                ...
                if cause is not None and cause.get("type") != "lifetime-eof":
                    group.kill()
                ...
                for key, _ in selector.select(0.05):
                    ...
                    if key.data == 1:
                        ...
                        if stderr_size + protocol.total > protocol.maximum:
                            raise policy.GuardError("external combined stdout/stderr bound exceeded")
                        if not protocol.ready and stderr_size <= policy.ERROR_BYTES:
                    ...
                    try:
                        ...
                        cause = cause or {"type": "output-bound" if mode == "output" else "protocol", "message": str(error)}
                        continue
                    if stderr_size + protocol.total > protocol.maximum:
                        cause = cause or {"type": "output-bound", "message": "external combined output bound exceeded"}
        ...
        result["output_bytes"] = protocol.total + stderr_size
...
def validate_probe(result):
    ...
    if mode in {"identity", "pids", "disk"}:
        ...
        if (result.get("first_cause") or {}).get("type") != "output-bound" or result["output_bytes"] <= 65536:
...
def main():
    ...
    if output.name != "issue180-ci-baseline-" + args.run_id or output.is_symlink():
