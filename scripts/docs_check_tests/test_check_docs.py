...
class StaleIssue5StatusAndGateNumberRegressionTests(unittest.TestCase):
    ...
    def test_verify_gates_item_expansion_entries_are_the_final_two_gates(self):
        ...
        self.assertGreaterEqual(len(all_gates), 2)
        self.assertIn("itemexpansion", all_gates[-2].name)
        self.assertIn("itemexpansion", all_gates[-1].name)
        for gate in all_gates[:-2]:
