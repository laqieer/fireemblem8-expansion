...
class DevelopmentWorkflowSkillTests(unittest.TestCase):
    ...
    def test_bounded_exact_sha_agent_handoff_is_indexed(self):
        ...
        for requirement in (
            ...
            "fixed `origin` ref",
            "append-only authority commit",
            "locally reset ref cannot forge genesis",
            "force-with-lease/CAS",
            "sequence zero and a null head",
            "omitted/truncated prior list",
            "bounded before/fetch/after transaction",
            "retry from the new OID",
            "fail `authority-moved`",
            "re-query it immediately before the eligibility decision",
        ...
        for requirement in (
            ...
            "fixed `origin` authority ref",
            "single-parent authority commits",
            "expected-head/sequence CAS",
            "remote-OID-before/fetch/remote-OID-after",
            "fail `authority-moved` on exhaustion",
            "rechecked at the final eligibility boundary",
